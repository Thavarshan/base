...
files=$(grep -E -r -l -i ":author|:vendor|:package|:short|thvarshan|author@domain.com|vendor_slug|base" --exclude-dir=vendor ./* ./.github/* | grep -v "$script_name")
...
files_regex=":author|:vendor|:package|:short|thvarshan|author@domain.com|vendor_slug|base"
grep -E -r -l -i "$files_regex" --exclude-dir=vendor ./* ./.github/* \
| grep -v "$script_name" \
| while read -r file ; do
    new_file="$file"
    new_file="${new_file//$ClassName}"
    new_file="${new_file//base/$package_slug}"
